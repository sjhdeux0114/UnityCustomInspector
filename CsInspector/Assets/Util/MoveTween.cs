...
public class MoveTween : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < waypoints.Count; i++)
        {
            ...
            Gizmos.DrawLine(waypoints[i].position, waypoints[i].position + waypoints[i].GetTangent() * (waypoints[i].strength * 0.5f));
            Gizmos.color = Color.cyan;
        }
        ...
        else if (pathType == PathType.CatmullRom)
        {
            for (int i = 0; i < waypoints.Count - 1; i++)
            {
                ...
                {
                    float t = n / 20f;
                    Vector3 currentPos = GetCatmullRomPoint(p0, p1, p2, p3, t);
        ...
        else if (pathType == PathType.CubicBezier)
        {
            for (int i = 0; i < waypoints.Count - 1; i++)
            {
                ...
                Vector3 prevPos = start.position;
                for (int n = 1; n <= 20; n++)
                {
                    float t = n / 20f;
                    Vector3 currentPos = GetBezierPoint(start.position, c1, c2, end.position, t);
                    Gizmos.DrawLine(prevPos, currentPos);
                    prevPos = currentPos;
                }
            }
